        ...
        with open(ns + '_pods.txt', "rt") as pod:
            for p in pod:
                ...
                with open(ns + '_' + p + '_containers.txt', "rt") as cont:
                    for c in cont:
                        c = c.strip("\n")
                        #cmd = 'kubectl exec ' + p + ' -n ' + ns + ' -c ' + c + r" -- ls -la /root"
                        #cmd = 'kubectl exec ' + p + ' -n ' + ns + ' -c ' + c + r" -- ls -la /etc/passwd"
                        cmd = 'kubectl exec ' + p + ' -n ' + ns + ' -c ' + c + r" -- ls -la /etc/shadow"
                        #cmd = 'kubectl exec ' + p + ' -n ' + ns + ' -c ' + c + r" -- ls -la /usr/bin/tcpdump"
